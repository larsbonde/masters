...
def lstm_quad_train(
    ...
    batch_size,
    device,
    collate_fn=pad_collate_chain_split_cdr,
    extra_print=None,
):
    train_losses = list()
    valid_losses = list()
...
def lstm_quad_predict(model, dataset, idx, device, collate_fn=pad_collate_chain_split_cdr):
...
out_dir = root / "state_files" / "tcr_binding" / "cdr3_and_pep_only"
